...
- registry-wide UI/API surface-contract tests for every exposed object type
...
For this plugin specifically, Lane A must also prove that a green suite means more than "the generators still build classes." It must validate that every object registered in the plugin exposes the correct behavior at the surface layer:
- list pages load for every object type
- detail pages load for every object type
- Add/Edit/Delete affordances appear only when the corresponding route and permission actually exist
- read-only objects never render broken Add/Edit/Delete links in page controls, row-action menus, or related-object tables
- API viewsets expose only the HTTP methods and custom actions their registry/viewset contract allows
...
### Immediate next sprint test target: provider write-through on Krill

The next sprint should treat Krill as the write-path proving ground for the provider workflow already present in the plugin. The validation goal is broader than testing a single POST call: it must prove that provider-backed reconciliation, change-plan preview, approval, apply, and post-apply re-sync all work as one operator flow.

This sprint should add the following test obligations.

Lane A coverage:
- plan-to-provider-delta transformation tests for create and withdraw items
- capability-gating tests proving that write actions are only exposed when a provider supports ROA write
- approval and apply state-machine tests for change plans and plan items
- outbound provider payload preview tests, including deterministic serialization of Krill `added` and `removed` arrays
- provider write audit-record tests covering success, partial failure, duplicate rejection, and unsupported-operation handling
- permission tests for preview, approve, and apply actions

Lane B coverage:
- live preview/apply validation against the local or public Krill-backed environment
- live create flow proving that a missing authorization can be materialized from a plan item
- live withdraw flow proving that an orphaned imported authorization can be removed from provider state
- post-apply sync verification proving that a fresh provider snapshot reflects the applied delta
- negative-path tests using Krill validation responses for duplicate adds, unknown removals, and invalid prefix/max-length combinations

The minimum acceptance gate for the sprint should be:
- deterministic offline tests for delta generation and workflow state
- live Krill tests for create, withdraw, and post-apply re-sync
- no reliance on ARIN or RIPE gated environments for core write-path development
...
The same idea should also exist one layer higher for the plugin's generated surfaces. In addition to provider-adapter contract tests, the suite should maintain registry-driven surface contract tests that iterate every exposed object spec and verify:
- UI list actions versus actual create support
- row-action menus versus actual edit/delete route availability
- generated detail-page actions versus actual object mutability
- API read-only versus writable method exposure

Without this layer, a suite can report green while still missing broken Add/Edit/Delete affordances on objects whose pages technically render under narrower permissions or whose routes were only smoke-tested structurally.

Use `devrun/work_in_progress/netbox_rpki_surface_contract_checklist.md` as the concrete release gate for these registry and generated-surface changes.
