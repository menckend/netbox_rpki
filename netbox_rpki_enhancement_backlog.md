...
**Bottom line:** the highest-value next step is to evolve the plugin from a passive RPKI object catalog into an **operator-facing intent reconciliation, compliance, and workflow layer**.
...
- `IntentSource` metadata on derived objects
...
- start read-only
- calculate expected state from NetBox IPAM + ASN associations + tags/custom fields
- add background jobs for recomputation
- provide UI tables plus API endpoints
...
**Implementation notes**
- phase 1: import only
- phase 2: preview export payloads
- phase 3: optionally support controlled write-back for approved changes
...
- later tie approved plans to provider export or write-back
...
### 6.3 Prefer read-only synchronization before write-back
...
5. only then consider write-back automation
...
If only three features are implemented next, they should be:

1. **intent-to-ROA reconciliation**
2. **provider synchronization**
3. **ROA linting and safety analysis**
...
- API serializer tests for new object families
- permission tests for approval and change-plan workflows
- UI smoke tests for key list/detail views
